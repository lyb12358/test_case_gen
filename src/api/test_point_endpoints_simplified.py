...
name_sync = NameSyncService(db_operations)
...
async def create_test_point(
    ...
    try:
        # Auto-generate test_point_id if not provided
        test_point_id = test_point.test_point_id
        ...
        logger.error(f"Error creating test point: {str(e)}")
        db.rollback()
        raise HTTPException(status_code=500, detail="Failed to create test point")
